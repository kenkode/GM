...
class Leaveapplication extends \Eloquent {
    ...
    public static function createLeaveRoster($data){

        for($i=0;$i<count(array_get($data, 'leavetype_id'));$i++){
        if((array_get($data, 'leavetype_id')[$i] != '' && array_get($data, 'leavetype_id')[$i] != null) && (array_get($data, 'applied_start_date')[$i] != '' && array_get($data, 'applied_start_date')[$i] != null) && (array_get($data, 'days')[$i] != '' && array_get($data, 'days')[$i] != null)){
		$organization = Organization::getUserOrganization();
    ...
    if(count(Supervisor::where('employee_id',array_get($data, 'employee_id'))) > 0){

        $leaveapplications = DB::table('leaveapplications')->where('employee_id', '=', array_get($data, 'employee_id'))->where('is_roster', '=', 1)->whereYear('application_date', '=', Input::get('period'))->get();

        $supervisor = Supervisor::where('employee_id',array_get($data, 'employee_id'))->first();
		...
		Mail::send( 'emails.leaveroster', array('leaveapplications'=>$leaveapplications, 'name'=>$name, 'employee'=>$emp, 'supervisor'=>$employee), function( $message ) use ($employee,$name)
		{
    		
    		$message->to($employee->email_office )->subject( 'Vacation Roster for '.$name.' for year '.date('Y'));
		});
	}

	if(count(Employee::where('employee_id',array_get($data, 'employee_id'))) > 0){
	...
	public static function approveLeaveApplication($data, $id){
		...
		Mail::send( 'emails.leaveapprove', array('application'=>$application, 'name'=>$name), function( $message ) use ($employee)
		{
    		...
    		$message->to($employee->email_office )->subject( 'Vacation Approval' );
		});
	...
	public static function approveEmployeeLeaveApplication($id,$startdate,$enddate){
		...
		$employeeid = DB::table('leaveapplications')->where('id', '=', $application->id)->pluck('employee_id');
  		...
  		$supervisor = Supervisor::where('employee_id',$employeeid)->first();

        $sup = Employee::where('id',$supervisor->supervisor_id)->first();


		$name = $employee->first_name.' '.$employee->middle_name.' '.$employee->last_name;

		Mail::send( 'emails.leaveapprove', array('application'=>$application, 'name'=>$name, 'supervisor'=>$sup), function( $message ) use ($employee)
		{
	...
	public static function RedeemLeaveDays($employee, $leavetype){
		...
		Earning::insert($employee->id, 'Vacation earning', 'redeemed leave days', $amount);
	}
	...
	public static function applicationreport($departmentid){
        ...
        $fileApplName = 'Vacation_Application_Report_'.$start.'_'.$end.'.pdf';
	...
	public static function approvereport($departmentid){
        ...
        $fileApprName = 'Approved_Vacation_Report_'.$start.'_'.$end.'.pdf';
        $filePath = 'app/views/temp/';
	...
	public static function rejectreport($departmentid){
        ...
        $fileRejName = 'Rejected_Vacation_Report_'.$start.'_'.$end.'.pdf';
        $filePath = 'app/views/temp/';
	...
	public static function employeereport($departmentid){
        ...
        $fileEmpName = 'Employee_on_Vacation_Report_'.$start.'_'.$end.'.pdf';
        $filePath = 'app/views/temp/';
	...
	public static function individualreport($departmentid,$employeeid){
        ...
        $fileIndName = $employee->first_name.'_'.$employee->last_name.'_Vacation_Report.pdf';
        $filePath = 'app/views/temp/';
